...
from dich_truyen.config import PipelineConfig  # noqa: E402
from dich_truyen.pipeline.streaming import (  # noqa: E402
    PipelineResult,
    PipelineStats,
    StreamingPipeline,
)
...
class TestStreamingPipelineIntegration:
    ...
    def book_with_raw_chapters(self, tmp_path):
        ...
        (raw_dir / "0001_第一章.txt").write_text(
            "# 第一章 开始\n\n这是第一章的内容。修炼是一条漫长的道路。", encoding="utf-8"
        )
        (raw_dir / "0002_第二章.txt").write_text(
            "# 第二章 修炼\n\n主角开始修炼。练气境是第一个境界。", encoding="utf-8"
        )
    ...
    async def test_pipeline_translates_metadata(self, book_with_raw_chapters):
        ...
        _engine = await setup_translation(
            book_dir=book_dir,
